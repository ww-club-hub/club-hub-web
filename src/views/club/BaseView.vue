...
<template>
  <section class="bg-white dark:bg-gray-900 grow">
    <div class="max-w-screen-2xl mx-auto p-4 flex flex-row gap-3">
      ...
      <!-- join sidebar -->
      <div class="m-3 border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 ms-auto rounded p-3">
        <template v-if="stuco || officer">
          <router-link :to="{ name: 'club-settings', params: { clubId } }" v-if="officer" class="w-full text-orange-700 hover:text-white border-2 border-orange-700 hover:bg-orange-800 focus:ring-4 focus:outline-none focus:ring-orange-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mb-2 dark:border-orange-500 dark:text-orange-300 dark:hover:text-white dark:hover:bg-orange-500 dark:focus:ring-orange-800">Club Settings</router-link>
          <router-link :to="{ name: 'club-dashboard', params: { clubId } }" class="w-full text-white bg-orange-600 hover:bg-orange-700 focus:ring-4 focus:outline-none focus:ring-orange-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-orange-600 dark:hover:bg-orange-700 dark:focus:ring-orange-800">Go to Dashboard</router-link>
        </template>
      </div>
    </div>
  </section>
